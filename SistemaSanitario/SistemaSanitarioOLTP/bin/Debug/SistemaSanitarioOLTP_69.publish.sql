...
BEGIN
    ...
    SELECT vm.codHospital,
           ...
           vm.DNI_Medico,
           vm.num_habitacion,
           vm.fecha_hora AS TiempoSK_FechaHora,  -- ⬅️ Se mantiene como DATETIME
           vm.fecha_alta AS TiempoSK_FechaAlta,  -- ⬅️ Se mantiene como DATE
           DATEDIFF(DAY, vm.fecha_hora, vm.fecha_alta) AS dias_ingreso,
           vm.diagnostico,
           vm.tratamiento-- ⬅️ Se calcula directamente
    FROM Pacientes.VisitaMedica vm
    LEFT JOIN Pacientes.HistoriaClinica hc 
        ON vm.codHist = hc.codHist
