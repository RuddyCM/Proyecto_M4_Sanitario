...
BEGIN
    ...
    SET fv.PacienteSK     = sv.PacienteSK,
        ...
        fv.fecha_hora     = CASE 
                                WHEN ISNUMERIC(CAST(sv.TiempoSK_FechaHora AS CHAR(8))) = 1 
                                THEN TRY_CONVERT(DATETIME, CAST(sv.TiempoSK_FechaHora AS CHAR(8)), 112) 
                                ELSE NULL 
                            END,
        fv.fecha_alta     = CASE 
                                WHEN ISNUMERIC(CAST(sv.TiempoSK_FechaAlta AS CHAR(8))) = 1 
                                THEN TRY_CONVERT(DATE, CAST(sv.TiempoSK_FechaAlta AS CHAR(8)), 112) 
                                ELSE NULL 
                            END,
