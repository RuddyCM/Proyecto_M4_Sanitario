...
BEGIN
    ...
    SELECT DISTINCT 
        CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, FechaHora)) +
                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, FechaHora)), 2) +
                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, FechaHora)), 2)))
        AS TiempoSK,
        FechaHora
    ...
    AND NOT EXISTS (
        ...
        WHERE dt.TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, FechaHora)) +
                                          RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, FechaHora)), 2) +
                                          RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, FechaHora)), 2)))
    );
    ...
    INSERT INTO BDSanitarioDW.dbo.Dim_Tiempo (TiempoSK, Fecha)
    SELECT DISTINCT 
        CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, FechaAlta)) +
                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, FechaAlta)), 2) +
                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, FechaAlta)), 2)))
        AS TiempoSK,
        FechaAlta
    FROM staging.Visitast
    ...
    AND NOT EXISTS (
        SELECT 1 FROM BDSanitarioDW.dbo.Dim_Tiempo dt 
        WHERE dt.TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, FechaAlta)) +
                                          RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, FechaAlta)), 2) +
                                          RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, FechaAlta)), 2)))
    );
    ...
    WHEN MATCHED THEN 
        ...
        SET fv.PacienteSK     = sv.PacienteSK,
            ...
            -- ✅ Convertimos FechaHora y FechaAlta a claves en Dim_Tiempo
            fv.TiempoSK_FechaHora = (SELECT TOP 1 TiempoSK FROM dbo.Dim_Tiempo 
                                     WHERE TiempoSK = 
                                        CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaHora)) +
                                                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, sv.FechaHora)), 2) +
                                                      RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, sv.FechaHora)), 2)))),

            fv.TiempoSK_FechaAlta = (SELECT TOP 1 TiempoSK FROM dbo.Dim_Tiempo 
                                     WHERE TiempoSK = 
                                        CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaAlta)) +
    ...
    WHEN NOT MATCHED THEN 
        ...
        VALUES (sv.VisitaSK, sv.PacienteSK, sv.MedicoSK, sv.HospitalSK, sv.ServicioSK, 
                (SELECT TOP 1 TiempoSK FROM dbo.Dim_Tiempo 
                 WHERE TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaHora)) +
                                                RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, sv.FechaHora)), 2) +
                                                RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, sv.FechaHora)), 2)))),
                
                (SELECT TOP 1 TiempoSK FROM dbo.Dim_Tiempo 
                 WHERE TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaAlta)) +
                                                RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, sv.FechaAlta)), 2) +
                                                RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, sv.FechaAlta)), 2)))),
