...
BEGIN
    ...
    UPDATE SET 
        ...
        fv.[fecha_hora]     = CONVERT(DATE, CAST(sv.[TiempoSK_FechaHora] AS CHAR(8)), 112),
        fv.[fecha_alta]     = CASE 
                                  WHEN sv.[TiempoSK_FechaAlta] = 0 THEN NULL 
                                  ELSE CONVERT(DATE, CAST(sv.[TiempoSK_FechaAlta] AS CHAR(8)), 112) 
                              END,
    ...
    VALUES (sv.[VisitaSK], sv.[PacienteSK], sv.[MedicoSK], sv.[HospitalSK], sv.[ServicioSK], sv.[num_habitacion], 
            CONVERT(DATE, CAST(sv.[TiempoSK_FechaHora] AS CHAR(8)), 112), 
            CASE 
                WHEN sv.[TiempoSK_FechaAlta] = 0 THEN NULL 
                ELSE CONVERT(DATE, CAST(sv.[TiempoSK_FechaAlta] AS CHAR(8)), 112) 
            END,
