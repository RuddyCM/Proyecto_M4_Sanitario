...
BEGIN
    ...
    -- 📌 1️⃣ Insertar fechas en Dim_Tiempo si no existen (para evitar errores de clave foránea)
    ...
    SELECT DISTINCT 
        ...
        AS TiempoSK,
        sv.FechaHora
    FROM staging.Visitast sv
    LEFT JOIN BDSanitarioDW.dbo.Dim_Tiempo dt 
        ON dt.TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaHora)) +
                                        RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, sv.FechaHora)), 2) +
                                        RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, sv.FechaHora)), 2)))
    WHERE dt.TiempoSK IS NULL AND sv.FechaHora IS NOT NULL;
    ...
    FROM staging.Visitast sv
    LEFT JOIN BDSanitarioDW.dbo.Dim_Tiempo dt 
        ON dt.TiempoSK = CONVERT(INT, (CONVERT(CHAR(4), DATEPART(YEAR, sv.FechaAlta)) +
                                        RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(MONTH, sv.FechaAlta)), 2) +
                                        RIGHT('0' + CONVERT(VARCHAR(2), DATEPART(DAY, sv.FechaAlta)), 2)))
    WHERE dt.TiempoSK IS NULL AND sv.FechaAlta IS NOT NULL;
