...
BEGIN
    ...
    -- Aseguramos que los datos coincidan en tipo antes de hacer la comparación
    MERGE INTO [dbo].[Dim_Servicio] AS ds
    USING (
        SELECT 
            TRY_CAST(idServicio AS INT) AS idServicio,  -- Convierte solo si es posible
            nombre_servicio
        FROM [staging].[Serviciost]
        WHERE TRY_CAST(idServicio AS INT) IS NOT NULL  -- Filtra valores no convertibles
    ) AS ss
    ...
    WHEN NOT MATCHED THEN
        INSERT ([ServicioSK], [nombre_servicio])
