...
BEGIN
    ...
    UPDATE SET 
        ...
        fv.[fecha_hora]     = TRY_CONVERT(DATE, CAST(sv.[TiempoSK_FechaHora] AS CHAR(8)), 112),
        fv.[fecha_alta]     = CASE 
                                  WHEN sv.[TiempoSK_FechaAlta] = 0 THEN NULL 
                                  ELSE TRY_CONVERT(DATE, CAST(sv.[TiempoSK_FechaAlta] AS CHAR(8)), 112) 
                              END,
    ...
    INSERT ([VisitaSK], [PacienteSK], [MedicoSK], [HospitalSK], [ServicioSK], [num_habitacion], [fecha_hora], [fecha_alta], [diagnostico], [tratamiento], [dias_ingreso])
    VALUES (sv.[VisitaSK], sv.[PacienteSK], sv.[MedicoSK], sv.[HospitalSK], sv.[ServicioSK], sv.[num_habitacion], 
            TRY_CONVERT(DATE, CAST(sv.[TiempoSK_FechaHora] AS CHAR(8)), 112), 
            CASE 
                WHEN sv.[TiempoSK_FechaAlta] = 0 THEN NULL 
                ELSE TRY_CONVERT(DATE, CAST(sv.[TiempoSK_FechaAlta] AS CHAR(8)), 112) 
            END,
